...
<style>
    ...
    .preview{max-height:90px;border:1px solid #ddd;border-radius:6px;padding:2px}
</style>
...
<div class="container mt-4">
    <h1 style="font-size:23px;font-weight:bolder;">Edit Banner</h1><br>

    <form action="{{ route('Dashboard.admin.banner.update', $banner->id) }}" method="POST" enctype="multipart/form-data">
        @csrf
        @method('PUT')
        ...
        <div class="row">
            <div class="col-md-4 mb-3">
                ...
                <img class="preview" src="{{ asset($banner->image_path) }}" alt="Banner">
            </div>
            <div class="col-md-4 mb-3">
                <label class="form-label">Change Image (optional)</label>
                <input type="file" name="image" class="form-control rounded-0 uniform-width" accept="image/*">
                @error('image')<div class="text-danger">{{ $message }}</div>@enderror
            </div>

            <div class="col-md-4 mb-3">
                <label class="form-label">Title</label>
                <input name="title" type="text" class="form-control rounded-0 uniform-width" value="{{ old('title', $banner->title) }}" required>
                @error('title')<div class="text-danger">{{ $message }}</div>@enderror
            </div>

            <div class="col-md-8 mb-3">
                <label class="form-label">Paragraph</label>
                <textarea name="paragraph" class="form-control rounded-0 uniform-width" rows="3" style="height:auto">{{ old('paragraph', $banner->paragraph) }}</textarea>
