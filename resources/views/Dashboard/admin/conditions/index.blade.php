...
<div class="container-fluid mt-3">
  <div class="page-wrap">
    ...
    <div class="page-head">
      <div>
        <h1>Conditions</h1>
        <p>Manage conditions, status and actions.</p>
      </div>

      <div class="head-right">
        <div class="searchbox">
          <i class="fas fa-search"></i>
          <input type="text" class="search-input" placeholder="Search condition, slug..." id="condSearch">
        </div>

        <a href="{{ asset('dashboard/conditions/add') }}" class="add-new-btn">
    ...
    <div class="card-shell">
      <div class="card-top">
        ...
        <span style="font-size:13px; color:rgba(15,23,42,.65); font-weight:700;">
          Showing {{ $conditions->firstItem() ?? 0 }} to {{ $conditions->lastItem() ?? 0 }}
        </span>
      </div>
            ...
            @forelse($conditions as $c)
              <tr>
                ...
                <td class="text-center">
                  <div class="actions">
                    <a class="icon-btn" href="{{ route('Dashboard.admin.conditions.edit',$c->id) }}">
                      <i class="fas fa-pen"></i>
                    </a>

                    <form action="{{ route('Dashboard.admin.conditions.delete', $c->id) }}" method="POST"
            ...
            @empty
              <tr>
                <td colspan="4" style="padding:26px 14px; text-align:center; color: rgba(15,23,42,.65);">
                  <div style="font-weight:900; color: var(--text); margin-top:6px;">No conditions found</div>
                </td>
              </tr>
            @endforelse
          </tbody>
...
<script>
  ...
  const condTable = document.getElementById('condTable');

  if(condSearch && condTable){
    condSearch.addEventListener('keyup', function(){
      const q = this.value.toLowerCase().trim();
      const rows = condTable.querySelectorAll('tbody tr');
