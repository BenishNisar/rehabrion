							...
							<tbody>
                                @forelse($jobs as $index => $job)
                                    <tr>
                                        ...
                                        <td>{{ $job->position }}</td>
                                        <td class="wspace-no">{{ $job->job_type ?? 'N A' }}</td>
                                        <td>{{ $job->posted_date ? \Carbon\Carbon::parse($job->posted_date)->format('d-m-Y') : 'N A' }}</td>
                                        <td>{{ $job->last_date_to_apply ? \Carbon\Carbon::parse($job->last_date_to_apply)->format('d-m-Y') : 'N A' }}</td>
                                        <td>{{ $job->close_date ? \Carbon\Carbon::parse($job->close_date)->format('d-m-Y') : 'N A' }}</td>
...
<a href="{{ route('Dashboard.admin.jobs.view', $job->id) }}" class="btn btn-success light me-2" title="View">
                                                ...
                                                <a href="{{ route('Dashboard.admin.jobs.edit', $job->id) }}" class="btn btn-secondary light me-2" title="Edit">
                                                ...
                                                <form action="{{ route('Dashboard.admin.jobs.delete', $job->id) }}" method="POST">
                                                    @csrf
                                                    @method('DELETE')
